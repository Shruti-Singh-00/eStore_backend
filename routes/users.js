        ...
        if (error) {
          ...
          if (resultCount[0].count > 0) {
            ...
            bcryptjs.hash(password, 10).then((hashedPassword) => {
              const query = `Insert into users (email,firstName ,lastName,address,city,state,pin,password) values (
    ...
    pool.query(
      ...
      (error, result) => {
        if (error) {
          ...
          if (result.length > 0) {
            bcryptjs
              .compare(password, result[0].password) //Returns Boolean Promise..
              .then((compareResult) => {
                if (compareResult) {
